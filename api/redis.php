...
class RedisServer {
	...
	private static $types = array(Redis::REDIS_STRING => 'string', Redis::REDIS_SET => 'set', Redis::REDIS_LIST => 'list', Redis::REDIS_ZSET => 'zset', Redis::REDIS_HASH => 'hash', Redis::REDIS_NOT_FOUND => 'other', );
	...
	public static function init($host = '127.0.0.1', $port = 6379, $password = false) {
		...
		echo 'b';
		$instance = new RedisServer($host, $port, $password);
		echo 'h';
	}
	...
	private function get_type($key) {
		$type = $this -> redis -> type($key);
		return array('type' => $type, 'type_str' => self::$types[$type], );
	}
	...
	private function human_size($size) {
		$size = sprintf("%f", $size);
		return ($size>>30) ? round($size / (1<<30), 1) . ' Gb' : (($size>>20) ? round($size / (1<<20), 1) . ' Mb' : (($size>>10) ? round($size / (1<<10), 1) . ' Kb' : ((int)$size)));
	}
